...
| `renderMode` | `"kitty" \| "cell"` | `undefined` | Renderer selection. `undefined` follows the cached graphics probe (`getKittyGraphicsSupported() === false` selects cell mode, `true` or `null` selects kitty), and `"cell"` forces the block-glyph fallback |
...
| `getRenderMode(): "kitty" \| "cell"` | Which rendering path is active, the Kitty graphics protocol or the block-glyph cell fallback |
...
- `detectCellGlyphMode`, `CellGlyphMode`: environment-based cell glyph strategy detection (Terminal.app gets background mode because its font-drawn block glyphs do not tile the cell)
