...
| `renderMode`          | `"kitty"`, `"cell"`, undefined            | probe   | Renderer selection. `undefined` follows the graphics probe, a string forces that renderer                                                                                                                                                        |
| `limitColors`         | `0`, `16`, `256`, undefined               | auto    | Cell-mode color depth, `0` means truecolor. `undefined` auto-detects from `COLORTERM`/`TERM`                                                                                                                                                     |
| `cellGlyphMode`       | `"half-block"`, `"background"`, undefined | auto    | Cell-mode glyph strategy. `"half-block"` packs 2 pixels per cell using U+2580, `"background"` packs 1 pixel per cell using background-colored spaces. `undefined` auto-detects from `TERM_PROGRAM` (Terminal.app gets `"background"`)            |
